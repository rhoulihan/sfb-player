...
- [ ] 🟠 **D19.26** `ssd-pipeline/viewer/battle.html:1822` — During the 4-impulse fire-control reactivation window the ship is barred from firing entirely, but D19.26 allows it to keep firing on passive fire control.
- [ ] 🟠 **D19.25** `ssd-pipeline/viewer/battle.html:1841` — A ship on passive fire control that is also using erratic maneuvers can still fire; D19.25 forbids any firing/launching in that state.
...
- [ ] 🟠 **D17.26** `ssd-pipeline/viewer/battle.html:1765` — Tactical-intel level ignores the target's ECM entirely — every ECM shift in the target's favor should drop the info level one step.
...
- [ ] 🟠 **D6.11** `ssd-pipeline/viewer/lock-on.js:7` — The lock-on roll threshold is hardcoded to 6 and never uses the firer's current sensor rating, so a sensor-damaged ship still auto-locks.
...
- [ ] 🟡 **M2.31 / M2.34** `ssd-pipeline/viewer/mines.js:23` — No mine-arming delay: a mine can detonate the same impulse it is laid.
